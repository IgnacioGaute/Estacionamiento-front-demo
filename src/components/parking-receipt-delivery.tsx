...
export function ParkingReceiptDelivery({ registrationId, kind, onDismiss, showDisabledMessage = false }: {
  ...
  const url = receipt && typeof window !== 'undefined' ? `${window.location.origin}/comprobantes/${receipt.token}` : '';
  return <Dialog open={!!registrationId && (!!receipt || !!error)} onOpenChange={open => { if (!open) onDismiss(); }}>
    <DialogContent className="max-h-[90dvh] overflow-y-auto sm:max-w-md">
      ...
      {error ? <div className="space-y-3"><p role="alert">{error}</p><Button onClick={() => setAttempt(value => value + 1)}>Reintentar comprobante</Button></div> : receipt && <>
        ...
        {receipt.settings.qr && <div className="space-y-2 text-center"><QRCodeSVG value={url} size={224} marginSize={4} className="mx-auto max-w-full rounded bg-white" title="Escaneá para ver el comprobante" /><p className="text-sm">Escaneá el QR con tu celular</p></div>}
